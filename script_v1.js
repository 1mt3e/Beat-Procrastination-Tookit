...
function switchTab(i) {
  ...
  if (document.startViewTransition && !pm()) {
...
function animateCount(id, target) {
  ...
  requestAnimationFrame(function update(now) {
    const elapsed = now - startTime, t = Math.min(elapsed / duration, 1), ease = 1 - Math.pow(1 - t, 4);
...
/* ── Footer Modal & Widgets ── */
function openFooterModal(event, title, renderFn) {
  event.preventDefault();
  const overlay = document.getElementById('footer-modal');
  const contentBox = document.getElementById('footer-modal-content');
  
  contentBox.innerHTML = `
    <h3 style="margin-bottom: 1.25rem; font-family: 'Outfit', sans-serif; font-size: 1.5rem; color: var(--navy);">${title}</h3>
    <div id="footer-widget-body"></div>
  `;
  
  renderFn(document.getElementById('footer-widget-body'));
  
  overlay.style.display = 'flex';
  // Small delay to ensure transition triggers after display:flex
...
/* 1. Cite Machine */
let citeState = { style: 'apa', author: '', title: '', publisher: '', year: '', url: '' };
function openCiteMachine(event) {
  openFooterModal(event, 'Cite Machine', (container) => {
    container.innerHTML = `
      <div class="cite-tabs">
        <button class="cite-tab-btn active" onclick="setCiteStyle('apa', this)">APA 7th</button>
        <button class="cite-tab-btn" onclick="setCiteStyle('mla', this)">MLA 9th</button>
        <button class="cite-tab-btn" onclick="setCiteStyle('harvard', this)">Harvard</button>
      </div>
      <div style="display:flex; flex-direction:column; gap:0.75rem;">
        <input type="text" id="cite-author" placeholder="Author (e.g. Smith, J.)" class="bpt-input" oninput="updateCitationOutput()">
        <div style="display:flex; gap:0.5rem;">
          <input type="text" id="cite-year" placeholder="Year" class="bpt-input" style="width:30%" oninput="updateCitationOutput()">
          <input type="text" id="cite-title" placeholder="Title of Book / Article" class="bpt-input" style="flex:1" oninput="updateCitationOutput()">
        </div>
        <input type="text" id="cite-publisher" placeholder="Publisher / Journal" class="bpt-input" oninput="updateCitationOutput()">
        <input type="text" id="cite-url" placeholder="URL (Optional)" class="bpt-input" oninput="updateCitationOutput()">
      </div>
      <div class="cite-preview-box" id="cite-output">Citation will appear here...</div>
      <button class="btn btn-primary" style="width:100%" onclick="copyCitation()">
        <i class="ti ti-copy"></i> Copy Citation
      </button>
    `;
    updateCitationOutput();
  });
}
function setCiteStyle(style, btn) {
  citeState.style = style;
  document.querySelectorAll('.cite-tab-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  updateCitationOutput();
}
function updateCitationOutput() {
  const author = document.getElementById('cite-author')?.value.trim() || 'Author';
  const year = document.getElementById('cite-year')?.value.trim() || 'Year';
  const title = document.getElementById('cite-title')?.value.trim() || 'Title';
  const publisher = document.getElementById('cite-publisher')?.value.trim() || '';
  const url = document.getElementById('cite-url')?.value.trim() || '';
  
  let out = '';
  if (citeState.style === 'apa') {
    out = `${author}. (${year}). <i>${title}</i>. ${publisher}. ${url}`;
  } else if (citeState.style === 'mla') {
    out = `${author}. <i>${title}</i>. ${publisher}, ${year}. ${url}`;
  } else if (citeState.style === 'harvard') {
    out = `${author}, ${year}. <i>${title}</i>. ${publisher}. Available at: ${url}`;
  }
  
  const outputBox = document.getElementById('cite-output');
  if (outputBox) outputBox.innerHTML = out;
  citeState.rendered = out;
}
function copyCitation() {
  const temp = document.createElement('div');
  temp.innerHTML = citeState.rendered;
  navigator.clipboard.writeText(temp.textContent || temp.innerText).then(() => {
    showToast('Citation copied to clipboard!');
  });
}

/* 2. Focus Soundscapes Synthesis Engine */
let audioCtx = null;
let soundNodes = { brown: null, rain: null, lofi: null };
let soundGains = { brown: null, rain: null, lofi: null };
let soundPlaying = { brown: false, rain: false, lofi: false };
...
function toggleSound(key, btnElem, rowElem) {
  initAudioContext();
  const icon = btnElem.querySelector('i');
  
  if (soundPlaying[key]) {
    // Stop
    soundNodes[key].pause();
    ...
    // Start
    if (!soundGains[key]) {
      soundGains[key] = audioCtx.createGain();
      soundGains[key].connect(audioCtx.destination);
      soundGains[key].gain.value = 0.5; // Default volume
    }
    
    if (!soundNodes[key]) {
      soundNodes[key] = new Audio('https://stream.zeno.fm/f3wvbbqmdg8uv'); // Free lofi stream fallback
      soundNodes[key].crossOrigin = "anonymous";
      soundNodes[key].loop = true;
      const track = audioCtx.createMediaElementSource(soundNodes[key]);
      track.connect(soundGains[key]);
    }
    soundNodes[key].play();
...
function changeVolume(key, val) {
  if (soundGains[key] && audioCtx) {
    // Linear to exponential scale for perceived volume
    soundGains[key].gain.setTargetAtTime(val * val, audioCtx.currentTime, 0.05);
  }
}

function openSoundscapes(event) {
  openFooterModal(event, 'Lofi Chill', (container) => {
    container.innerHTML = `
      <p style="color:var(--muted); font-size:0.9rem; margin-bottom:1.5rem;">Play ambient Lofi chill beats to enhance your focus.</p>
      
      <div class="sound-row ${soundPlaying.lofi ? 'active' : ''}" id="row-lofi">
        <div class="sound-info"><i class="ti ti-device-audio-tape"></i> Lofi Radio Stream</div>
        <div class="sound-controls">
          <input type="range" class="vol-slider" min="0" max="1" step="0.05" value="0.5" oninput="changeVolume('lofi', this.value)">
          <button class="btn btn-outline" style="padding:0.4rem;" onclick="toggleSound('lofi', this, document.getElementById('row-lofi'))">
            <i class="ti ${soundPlaying.lofi ? 'ti-player-pause' : 'ti-player-play'}"></i>
          </button>
        </div>
      </div>
    `;
...
function openResearchTips(event) {
  openFooterModal(event, 'Research Tips', (container) => {
    container.innerHTML = `
      <ul style="padding-left:1.25rem; font-size:0.95rem; color:var(--text); line-height:1.7;">
        <li><b>"Exact Phrase"</b>: Use quotes to search for exact terms.</li>
        <li><b>AND / OR</b>: Combine keywords (e.g. <i>Cognitive AND Psychology</i>).</li>
        <li><b>filetype:pdf</b>: Add this to Google queries to find direct PDF documents.</li>
        <li><b>site:.edu</b>: Restrict your search to academic institution websites only.</li>
        <li><b>Google Scholar</b>: Always check the "All XX versions" link to find free un-paywalled PDFs.</li>
      </ul>
    `;
  });
}
...
function openTerms(event) {
  openFooterModal(event, 'Terms of Service', (container) => {
    container.innerHTML = `
      <p style="font-size:0.9rem; color:var(--muted); line-height:1.6;">
        Beat Procrastination Toolkit is a free, local-first utility designed to help students maximize their focus. All data is stored entirely in your browser's LocalStorage. No data is sent to external servers, and no tracking is employed.
      </p>
    `;
  });
}
